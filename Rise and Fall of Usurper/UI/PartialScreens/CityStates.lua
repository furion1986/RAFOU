...
--	Annex City-State
-- ===========================================================================
function OnAnnexCityState(localPlayerID:number, cityStateID, pEnvoyTokens:number, iAnnexCost:number)
	--Get Players
	local pPlayer = Players[localPlayerID];
	local iPlayerID = cityStateID;
	local iPlayer = Players[cityStateID];
	--Get Cities
	local iPlayerCities:table = iPlayer:GetCities();
	local iCities:table = {};
	for _, iCity in iPlayerCities:Members() do
		--Get Location
		local locX:number = iCity:GetX();
		local locY:number = iCity:GetY();
		--Get Popluation
		...
		--Get Plots and Improvements
		local iCityPlots:table = Map.GetCityPlots():GetPurchasedPlots( iCity );
		local plotImprovements:table = {};
		if (iCityPlots ~= nil) then
			for _,plotID in pairs(iCityPlots) do
				local iPlot:table =  Map.GetPlotByIndex(plotID);
				local iPlotImprovement:number = iPlot:GetImprovementType();
				--Almost sure that GetImprovementType() will return an index of the improvement.
				table.insert(plotImprovements, { 
					cityPlot = iPlot, 
		...
		table.insert(iCities, {
			CityX=locX,
			CityY=locY,
			CityPopulation=iPopulation,
			CityPlots=plotImprovements,
			CityDistricts=builtDistricts,
			CityBuildings=builtBuildings,
			CityName=iName});
	end
	--Get Units
	local iPlayerUnits:table = iPlayer:GetUnits();
	local builtUnits:table = {};
	for i, iUnit in iPlayerUnits:Members() do
		local unitType = GameInfo.Units[iUnit:GetUnitType()];
		local unitX = iUnit:GetX();
		local unitY = iUnit:GetY();
		table.insert(builtUnits, {
			UType=unitType,
			UX=unitX,
			UY=unitY});
	end
...
function ViewCityState( iPlayer:number )
	...
	if GameCapabilities.HasCapability("CAPABILITY_MILITARY") then
		...
		if pPlayerCities:GetCount() > 1 then 
			-- Might change in the future; currently city states will just raze 
			UI.DataError("The CityState player "..tostring(iPlayer).." has "..tostring(pPlayerCities:GetCount()).." cities, but should only have 1.");
		end
		for _, pCity in pPlayerCities:Members() do
		...
		local iBalance	:number = pLocalPlayer:GetTreasury():GetGoldBalance();
		local iLeaderIsTully = false;
		local canAnnexCityState = false;
		if (iLeader == "LEADER_HOSTER_TULLY") then
			iLeaderIsTully = true;
		end
		if (kCityState.IsLocalPlayerSuzerain) and (iBalance > iAnnexCost) then
			canAnnexCityState = true;
		end
		Controls.HosterTullyAnnexButton:SetHide(not iLeaderIsTully);
		Controls.HosterTullyAnnexButton:SetDisabled(not canAnnexCityState);
		if not kCityState.IsLocalPlayerSuzerain then
			local annexTooltip = Locale.Lookup("LOC_HT_CITY_STATES_NOT_SUZERAIN");
			Controls.HosterTullyAnnexButton:SetToolTipString(annexTooltip);
		else
			local annexTooltip = Locale.Lookup("LOC_HT_CITY_STATES_ANNEX_DETAILS", iAnnexCost);
			Controls.HosterTullyAnnexButton:SetToolTipString(annexTooltip);
		end
		local pPlayerInfluence:table	= pPlayer:GetInfluence();	
		local pEnvoyTokens = pPlayerInfluence:GetTokensReceived(pLocalPlayer:GetID());		
		Controls.HosterTullyAnnexButton:RegisterCallback( Mouse.eLClick, function() OnAnnexCityState( localPlayerID, iPlayer, pEnvoyTokens, iAnnexCost); end );
	else
		Controls.LevyMilitaryButton:SetHide(true);
		Controls.PeaceWarButton:SetHide(true);
		Controls.HosterTullyAnnexButton(true);
	end
